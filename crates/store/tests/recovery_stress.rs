...
use core_types::{BlobKey, PlacementClass, SegmentFileState, SegmentId, StrataLsn};
...
fn truncate_unsealed_tail(
    ...
    unsealed: &[(SegmentId, core_types::SegmentState)],
    run_id: u64,
) -> Option<String> {
...
fn create_orphan_segment_file(
    cfg: &StrataStoreConfig,
    states: &[(SegmentId, core_types::SegmentState)],
    run_id: u64,
) -> Option<String> {
    let existing = states
        .iter()
