...
use gc_planner::{DestinationClass, GcAction, GcCopyRecord, GcCopySelector, GcPlan, GcPlanner};
use index::StrataIndex;
use segment::{SegmentIoObserver, SegmentWriter};
...
impl GcExecutor {
    ...
    fn copy_gc_source_segment_to_staging(
        ...
        while offset < sealed_len {
            ...
            let header = DecodedRecord::peek_fixed_header(&fixed).map_err(segment::Error::from)?;
            let record_len = header.encoded_record_len().map_err(segment::Error::from)?;
            ...
            let payload_len =
                usize::try_from(header.payload_len).map_err(|_| segment::Error::RangeOverflow)?;
