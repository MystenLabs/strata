...
//! Record v1 is append-friendly and self-validating:
//!
//! ```text
//! +----------------------+----------------------+----------------------+
//! | fixed header (40 B)  | payload (N bytes)    | blob key (K bytes)   |
//! +----------------------+----------------------+----------------------+
//!
//! fixed header:
//!   magic | version | header_len | key_len | logical_end_epoch
//!   generation | payload_len | record_checksum
//! ```
//!
//! The record checksum is computed over:
//!
//! ```text
//! header(record_checksum = 0) || payload || key
//! ```
//!
//! Core invariants:
//!
//! - `BlobKey` is non-empty and bounded by the v1 record format limit.
