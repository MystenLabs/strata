...
fn map_ref_rewrites_exact_tail_payload_ref() {
    ...
    let mut state = PutState::default();
    append_put(&mut state, SHARD, put_entry(7, source));

    state.apply_merge_op(PutMergeOp::MapRef {
        publish_lsn: 11,
        shard: SHARD,
        payload_lsn: 7,
        from: source,
...
fn map_ref_does_not_make_relocated_payload_latest() {
    ...
    let mut state = PutState::default();
    append_put(&mut state, SHARD, put_entry(7, source));
    append_put(&mut state, SHARD, put_entry(9, newer));
    ...
    assert_eq!(
        state.entry_at_lsn(SHARD, 7).unwrap().record_ref,
        Some(destination)
    );
}
...
fn rollback_from_removes_map_ref_by_publish_lsn() {
    ...
    state.apply_merge_op(PutMergeOp::RollbackFrom { lsn: 11 });
