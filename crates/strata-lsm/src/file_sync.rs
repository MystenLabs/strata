...
pub type FileSyncSender = SyncSender<FileSyncTask>;
...
/// Creates a bounded task channel and a cloneable worker.
pub fn file_sync_channel(capacity: usize) -> (FileSyncSender, FileSyncer) {
...
mod tests {
    ...
    fn workers_share_the_receiver_and_complete_every_task() {
        ...
        let (task_tx, syncer) = file_sync_channel(4);
        let workers = (0..2)
            .map(|_| {
