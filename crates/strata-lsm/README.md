...
`strata-segment` owns checksummed value records and the reader, writer, and crash-prefix scanner. It
remains independently usable and is re-exported as `strata_lsm::segment` for compatibility, but
`Lsm` does not open or write segment files.

`strata-lsm` contains no WAL or file-sync queue. `strata-store` owns the one rolling WAL and encodes
