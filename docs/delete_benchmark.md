...
- `steady`: issue a deterministic, rate-limited mix of puts, payload gets, and deletes across the
  configured post-delete workers. Generated deletes target keys created by each worker, preventing
  repeated deletion of the initial key set. The default 40/20/40 mix is approximately live-data
  neutral, although early deletes can become puts when no generated live key is available. Reported
  actual operation counts are authoritative.
...
Use this Grafana query for net reclaim throughput:

```promql
rate(strata_store_gc_reclaimed_bytes_total{job="strata-bench"}[5m])
```

Use `increase(...[$__range])` instead to show total bytes reclaimed over the selected benchmark
window. GC deletion is bursty, so a multi-minute rate window is usually more useful than an
instantaneous rate.
